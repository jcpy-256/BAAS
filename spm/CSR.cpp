...
namespace SPM
{
  ...
  void CSR::alloc(int m, int nnz, bool createSeparateDiagData /*= true*/)
  {
    ...
    memset(colidx, 0, (nnz) * sizeof(int));
    memset(diagptr, 0, (m) * sizeof(int));
  ...
  CSR::CSR(const CSR &A) : m(A.m), n(A.n), values(NULL), idiag(NULL), diag(NULL), diagptr(NULL), extptr(NULL), ownData_(true)
  {
    ...
    if (diagptr)
      copyVector(diagptr, A.diagptr, m);
    if (idiag)
      copyVector(idiag, A.idiag, m);
    if (diag)
      copyVector(diag, A.diag, m);
  }
  ...
  CSR::CSR(const char *fileName, bool isLower, int base) : extptr(NULL)
  {

    COO Acoo;
    // loadMatrixMarket((char *)fileName, Acoo, forceSymmetric, pad); //
    loadLowerMatrixMarket((char *)fileName, Acoo); // using mmio_highlevel_coo read all matrix and add a major diagonal
    printf("m:%d, n:%d, nnz:%d\n", Acoo.m, Acoo.n, Acoo.nnz);
    int nnz = Acoo.nnz;
    // printf("end element row: %d, col: %d, val: %lf\n", Acoo.rowidx[nnz-1], Acoo.colidx[nnz -1], Acoo.values[nnz - 1]);
    alloc(Acoo.m, Acoo.nnz);
    dcoo2csr(this, &Acoo, base);
  }

  CSR::CSR(int m, int n, int *rowptr, int *colidx, double *values) : m(m), n(n), rowptr(rowptr), colidx(colidx), values(values), idiag(NULL), diag(NULL), diagptr(NULL), extptr(NULL), ownData_(false)
  {
...
#pragma omp parallel for
  ...
  int *CSR::constructDiagPtr_(int m, const int *rowptr, const int *colidx, int base)
  {
    if (0 == base)
    {
  ...
  // matrix permutation, csc has been alloc using a constructor
  void CSR::transpositionToCSC(const CSR *csrA, CSC *&cscA)
  {
    int *colidx = csrA->colidx;
    int *rowptr = csrA->rowptr;
    double *val_csr = csrA->values;
    int m = csrA->m;
    int n = csrA->n;
    int nnz = csrA->getNnz();
    if (cscA->isalloc())
    {
      dcsr2csc(m, n, nnz, rowptr, colidx, val_csr, cscA->colptr, cscA->rowidx, cscA->values);
    }
    else
    { // alloc memory space
      cscA->alloc(n, nnz, true);
      dcsr2csc(m, n, nnz, rowptr, colidx, val_csr, cscA->colptr, cscA->rowidx, cscA->values);
    }
  }
  ...
  void dcsr2csc(int m, int n, int nnz, int *rowptr, int *colidx, double *val_csr, int *colptr, int *rowidx, double *val_csc)
  {
    ...
    vectorExclusiveScan(colptr, 0, n);
    // printf("nnz: %d\n", nnz);
    assert(colptr[n] == nnz);
...
#pragma omp parallel for
    for (int i = 0; i < n; i++)
    {
      quicksort_pair(rowidx, val_csc, colptr[i], colptr[i + 1] - 1);
    }
  }
